...
<div id="paymentCodeContainer"></div>
	...
	<script>
    var qrCodeDisplayed = false; // Biến kiểm tra xem mã QR đã được hiển thị hay chưa
    ...
    function handlePayment() {
        ...
        if (paymentMethod === "online_payment" && (!fullname || !address)) {
            // Nếu chọn thanh toán trực tuyến nhưng chưa nhập đủ thông tin
            alert("Vui lòng nhập đủ thông tin để hiển thị mã QR.");
            return; // Dừng xử lý tiếp theo
        }

        // Kiểm tra nếu mã QR chưa được hiển thị
        if (!qrCodeDisplayed && paymentMethod === "online_payment") {
            // Tạo mã code thanh toán trực tuyến
            ...
            // Hiển thị mã code thanh toán
            document.getElementById("paymentCodeContainer").innerHTML = paymentCode;

            qrCodeDisplayed = true; // Đánh dấu là mã QR đã được hiển thị
        }
    }
</script>
